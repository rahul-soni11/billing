...
@if (!Session::has('party_name'))
<div class="alert alert-danger">
  <strong>Party is not selected !</strong> Please select the Party.
</div>
@endif
...
<hr>
              ...
              {{ csrf_field() }}
              <input type="hidden" name="party_id" value="{{Session::get('party_id')}}">
              <input type="hidden" name="category" value="@if(Request::is('purchase/new/gold')){{'gold'}}@elseif(Request::is('purchase/new/silver')){{'silver'}}@elseif(Request::is('purchase/new/diamond')){{'diamond'}}@elseif(Request::is('purchase/new/gemstone')){{'gems'}}@endif"> 
              <div class="card card-outline-secondary">
                  
                  <div class="card-header 
                    @if(Request::is('purchase/new/gold'))
                        {{'goldimg'}}
                    @elseif(Request::is('purchase/new/silver'))
                        {{'silvimg'}}
                    @elseif(Request::is('purchase/new/diamond'))
                        {{'diaimg'}}
                    @elseif(Request::is('purchase/new/gemstone'))
                        {{'gemimg'}}
                    @endif
                  ">
                    <h3 class="mb-0">
                      @if(Request::is('purchase/new/gold'))
                          {{'Gold Jewellery Purchase Record'}}
                      @elseif(Request::is('purchase/new/silver'))
                          {{'Silver Jewellery Purchase Record'}}    
                      @elseif(Request::is('purchase/new/diamond'))
                          {{'Diamond Jewellery Purchase Record'}}
                      @elseif(Request::is('purchase/new/gemstone'))
                          {{'Gemstones Purchase Record'}}
                      @endif
                    </h3>
                  </div>
                  <div class="card-body" >
                              ...
                              <div class="row ">
                                <div class="form-group col-md-3">
                                  ...
                                  <input name="party" type="text" class="form-control form-control-lg" id="party" readonly placeholder="Required"
                                  @if (Session::has('party_name'))
                                        value="{{Session::get('party_name')}}"
                                  @endif
                                  >
                              ...
                              <div class="row">
                                ...
                                <div class="form-group col-md-4">
                                  ...
                                  <input name="gst_no" class="form-control form-control-lg" id="gst_no" type="text" placeholder="Optional" maxlength="15"     @if (Session::has('party_gst_no'))value="{{Session::get('party_gst_no')}}"@endif readonly>
                                </div>
                              </div>
